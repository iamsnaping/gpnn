...
class VisualLocal(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        return frames,bbx,mask,label,cls_ids,cls_cls,rel[:,1:,:],private_label,torch.tensor([common_ans],dtype=torch.long),\
            token_tensor,mask_,frame_ans,frame_flag,torch.tensor(indices,dtype=torch.long),key,bbx_
...
class MixShift(Dataset):
    def __init__(self,name,sample_each_clip=16,node_nums=10,mapping_type=2,train=True):
        ...
        self.mapping_type=mapping_type
        self.json=json.load(
            open(os.path.join("/home/wu_tian_ci/GAFL/json_dataset/all_cls_rel/shift",name+str(mapping_type)+'.json'),'r')
        )

        self.mapping=json.load(
                open(os.path.join('/home/wu_tian_ci/GAFL/json_dataset/mapping_expand2/shift',name+str(mapping_type)+'.json'))
            )

        self.mask=json.load(
            open(os.path.join("/home/wu_tian_ci/GAFL/json_dataset/all_cls_rel/shift",name+str(mapping_type)+'_mask.json'),'r')
        )
        self.bbx=json.load(
            open(os.path.join("/home/wu_tian_ci/GAFL/json_dataset/all_cls_rel/shift",name+str(mapping_type)+'_bbx.json'),'r')
        )
        self.cls=json.load(
            open(os.path.join("/home/wu_tian_ci/GAFL/json_dataset/all_cls_rel/shift",name+str(mapping_type)+'_obj_cls.json'),'r')
        )
        self.rel=json.load(
            open(os.path.join("/home/wu_tian_ci/GAFL/json_dataset/all_cls_rel/shift",name+str(mapping_type)+'_rel.json'),'r')
        )
        # keys=list(self.json.keys())
        # self.keys=[item for item in keys if 'label' not in item]
        # self.label=[i for i in keys if 'label' in i]
        self.video_path=os.path.join('/home/wu_tian_ci/GAFL/data/hdf5/all_cls_rel/shift',name+str(mapping_type)+'.hdf5')
        self.video2size=json.load(open('/home/wu_tian_ci/revisiting-spatial-temporal-layouts/data/video2size/ag.json','r'))
        self.num_cls=157
        self.obj_cls_num=38
        self.rel_num=30
