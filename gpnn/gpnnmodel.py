...
sys.path.append('/home/wu_tian_ci/GAFL/gpnn')
sys.path.append('/home/wu_tian_ci/GAFL')
from torch_sparse import SparseTensor
from gpnn.layer import GNN, MLP
from gpnn.layer import ParsingNet_GPU as ParsingNet
from gpnn.layers.gcn_edge import GNN as GNN_edge
from gpnn.pool import global_mean_pool
import yaml
...
class GPNN(Module):
    ...
    def forward(self, data):
        ...
        edge_index = data.edge_index # shape: [num_nodes, num_nodes]
        adj_t=SparseTensor(row=edge_index[1],col=edge_index[0],value=data.edge_attr).to(h.device)
...
if __name__=='__main__':
    ...
    yaml_path='/home/wu_tian_ci/GAFL/gpnn/config/config.yaml'
    config= OmegaConf.load(yaml_path)
    # print(config)
    print(config['params_config']['task'])
    gpnn=GPNN(config['params_config'])